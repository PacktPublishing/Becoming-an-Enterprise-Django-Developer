...
class TestingCalibrator(SimpleTestCase):
    '''
    Calibrator, used to see if testing system works and is used as a boilerplate for creating new test classes.
    '''
    def setUp(self):
        '''
...
#    def setUp(self):
#        '''
#        setUp() tests if we are able to create all model objects, by creating a seller, we create all of the relate objects.
#        If successful, the Vehicle_Model, Engine, Vehicle and Seller objects all work as they should.
#        '''
#        model = Vehicle_Model.objects.create(name='Grand Cherokee Laredo 4WD', make=8)
#        engine = Engine.objects.create(name='3.6L FI FFV DO', vehicle_model=model)
#        vehicle = Vehicle.objects.create(vin='aa890123456789012', sold=False, price=Money(39875, 'USD'), make=8, vehicle_model=model, engine=engine)
...
#        '''
#        Checks if the full vehicle name of the vehicle created in the setUp() method is generated as it should.
#        Expected value = 'Jeep Grand Cherokee Laredo 4WD - 3.6L FI FFV DO'
#        '''
#        vehicle_1 = Vehicle.objects.get(vin='aa890123456789012')
#        self.assertEqual(vehicle_1.full_vehicle_name(), 'Jeep Grand Cherokee Laredo 4WD - 3.6L FI FFV DO') # Correct Value
...
#        #self.assertEqual(vehicle_2.full_vehicle_name(), 'Chevrolet Blazer LT - 3.9L DI DOHC 6cyl')
...
#        self.client = Client()
#        # Note - It would be wise to keep this password in your .env file as a TESTUSER_PASSWORD, then bring it in as a global variable in your settings.py file and then import your settings into this file to use your variable that way.
#        self.client.login(username=self.user.username, password='mynewpassword') # Correct Value
#        #self.client.login(username=self.user.username, password='mynewpassword1') # Incorrect Value

#    def test_get(self):
#        '''
#        Tests a custom-built REST-API Endpoint using the GET Method at the path http://localhost:8000/chapter-8/seller/1/, while using the Django Client() class to login for authentication.
#        Checks if it actually exists and returns a 200 response code (Valid)
#        '''
#        response = self.client.get('/chapter-8/seller/1/')

#        self.assertEqual(response.status_code, 200)

#        # Check that the returned context object has a seller name equal to what is expected (Test Biz Name)
...
#    def test_post(self):
#        '''
#        Tests creating an Engine at a Django REST Framework REST-API Endpoint using the POST method at http://localhost:8000/chapter-8/engines/, while using the Django REST-Framework APIClient() class to login for authentication. Checks if it returns a 201 response code (Created).
#        '''
...
#    def test_put(self):
#        '''
#        Tests updating an Engine at a Django REST Framework REST-API Endpoint using the PUT method at http://localhost:8000/chapter-8/engines/1/, while using the Django REST-Framework APIClient() class to login for authentication. Checks if it returns a 200 response code (Success).
#        '''
...
# This class was auto generated when you created a new Django app, either with Visual Studio or through the command-line.
